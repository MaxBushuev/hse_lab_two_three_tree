class Node(object):
    ...
    def insertValue(self, a):
        ...
        if a is not None and self.valcnt < 3:
            if self.valcnt == 0: self.values = [None] * 3
...
class TwoThreeTree(object):
    ...
    def __iter__(self):
        ...
        while len(stack):
            node = stack.pop()
            yield node    
            for j in range(node.refcnt - 1, -1, -1):
    ...
    def getPrev(self, a):
        node = self.findNode(a)
        values = [i for i in node.values if i is not None]

        if node.isLeafNode():
            if a == max(values) and len(values) == 2:
                return min(values)
            ...
            while True:
                ...
                links = [i for i in node.links if i is not None]
                values = [i for i in node.values if i is not None]
                
                if len(links) == 3:
                    if a in links[2].values:
                        return max(values)
                    elif a in links[1].values:
                        return min(values)
                    else:
                        a = min(values)
                        continue
                else:
                    if a in node.links[1].values:
                        return max(values)
                    else:
                        a = min(values)
                        continue
            ...
            links = [i for i in node.links if i is not None]
            if a == min(values):
                temp = [i for i in links[0].values if i is not None]
                return max(temp)
            else:
                temp = [i for i in links[1].values if i is not None]
                return max(temp)
    ...
    def getNext(self, a):
        ...
        if node.isLeafNode():
            if a == min(values) and len(values) == 2:
                return max(values)
...
if __name__ == "__main__":
    ...
    node = tree.findNode(16)
    print(tree.findNext(20))
